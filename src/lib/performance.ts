...
// ========== VÉLO : meilleures puissances moyennes (proxy par durée) ==========
// On accepte une activité si sa durée est dans une fenêtre proche de la cible.
function bestPowerForDuration(activities: Activity[], targetMin: number, label: string): BestEffort {
  const targetSec = targetMin * 60;
  const minSec = targetSec * 0.8;
  const maxSec = targetSec * 1.5;
  ...
  if (candidates.length === 0) {
    return { label, value: null, unit: "W", formatted: "—", date: null, activityName: null, activityId: null, insufficient: true, reason: `Aucune activité vélo de ~${targetMin} min avec puissance` };
  }
...
// ========== NATATION : meilleure allure /100m sur distances proxy ==========
function bestSwimPaceForDistance(activities: Activity[], targetM: number, label: string): BestEffort {
  const minM = targetM * 0.9;
  const maxM = targetM * 1.4;
  const candidates = activities
    .filter(isSwim)
    .filter((a) => a.distance_meters && (a.moving_time_seconds || a.duration_seconds))
  ...
  const best = withPace.reduce((x, y) => (x.pace < y.pace ? x : y));
  return { label, value: best.pace, unit: "/100m", formatted: fmtPace100m(best.pace), date: best.a.start_date, activityName: best.a.name, activityId: best.a.id };
}

export function computeBestEfforts(activities: Activity[]) {
  return {
    cycling: [
      bestPowerForDuration(activities, 1, "Puissance ~1 min"),
    ...
    running: [
      bestRunPaceForDistance(activities, 1, "1 km"),
      bestRunPaceForDistance(activities, 5, "5 km"),
      bestRunPaceForDistance(activities, 10, "10 km"),
      bestRunPace1h(activities),
      bestRunPaceForDistance(activities, 21.1, "Semi"),
    ],
    swimming: [
      bestSwimPaceForDistance(activities, 100, "100 m"),
      bestSwimPaceForDistance(activities, 400, "400 m"),
